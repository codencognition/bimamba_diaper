...
def pad_labels(ts: torch.Tensor, out_size: int) -> torch.Tensor:
    ...
    for _, t in enumerate(ts):
        if t.shape[1] < out_size:
            ...
            ts_padded.append(
                torch.cat(
                    (t, -1 * torch.ones((t.shape[0], out_size - t.shape[1]))), dim=1
                )
            )
    ...
    for _, t in enumerate(ts):
        if t.shape[1] < out_size:
            ...
            if t[-1, 0] == -1:
                ...
                beg = torch.cat(
                    (t[:boundary, :], torch.zeros((boundary, out_size - t.shape[1]))),
                    dim=1,
                )
                ...
                ts_padded.append(
                    torch.cat(
                        (
                            t,
                            torch.zeros(
                                (t.shape[0], out_size - t.shape[1]), device=t.device
                            ),
                        ),
                        dim=1,
                    )
...
def pad_sequence(
    features: List[torch.Tensor], labels: List[torch.Tensor], seq_len: int
    ...
    for i, _ in enumerate(features):
        ...
        if length < seq_len:
            ...
            features_padded.append(
                torch.cat(
                    (features[i], -torch.ones((extend, features[i].shape[1]))), dim=0
                )
            )
            labels_padded.append(
                torch.cat((labels[i], -torch.ones((extend, labels[i].shape[1]))), dim=0)
            )
        elif length > seq_len:
            raise (
...
def pit_loss_multispk(
    ...
    spk_labels = torch.from_numpy(
        np.asarray(
            [[1.0] * n_spk + [0.0] * (target.shape[2] - n_spk) for n_spk in n_speakers]
        )
    )
    ...
    for i in range(logits.shape[0]):
        ...
        boundary = min(
            torch.cat(
                (
                    torch.where(target[i, :, 0] == -1)[0].to("cpu"),
                    torch.tensor([target[i].shape[0]]),
                )
            )
        )
        if args.use_detection_error_rate:
            cost_mx = torch.matmul(
                1 - torch.sigmoid(logits_t[i, :, :boundary]), target[i, :boundary, :]
            ) + torch.matmul(
                torch.sigmoid(logits_t[i, :, :boundary]), 1 - target[i, :boundary, :]
            )
        else:
            cost_mx = (
                ...
                - logsigmoid(-logits_t[i, :, :boundary].unsqueeze(0)).bmm(
                    1 - target[i, :boundary, :].unsqueeze(0)
                )
            )[0]
    ...
    activation_loss = torch.sum(activation_loss, axis=1) / (target_matched != -1).sum(
        axis=1
    )
    ...
    activation_loss_DER = (miss + fa + conf) / target_matched[
        torch.where(target_matched > 0)
    ].sum()
...
def vad_loss(logits: torch.Tensor, ts: torch.Tensor) -> torch.Tensor:
    ...
    loss = torch.sum(loss, axis=1) / (torch.mean(ts, axis=2) != -1).sum(
...
def get_nooverlap_probs(p_sil: torch.Tensor):
    ...
    mask = torch.cat(
        (
            torch.zeros(1, p_sil.shape[2], device=p_sil.device),
            torch.eye(p_sil.shape[2], device=p_sil.device),
        ),
        dim=0,
    )
    prob_mask = (
        mask.unsqueeze(0).unsqueeze(0).expand(p_sil.shape[0], p_sil.shape[1], -1, -1)
    )
    inv_prob_mask = 1 - prob_mask
    p_nooverlap = torch.sum(
        torch.prod(
            prob_mask * (1 - p_sil).unsqueeze(2) + inv_prob_mask * p_sil.unsqueeze(2),
            dim=3,
        ),
        dim=2,
    ).unsqueeze(2)
...
def get_loss(
    ...
    (activation_loss, activation_loss_DER, attractor_existence_loss, permutations) = (
        pit_loss_multispk(logits_padded, ts_padded, attractors_logits, n_speakers, args)
    )
    if not (args.speakerid_loss == ""):
        spkid_loss = speaker_identification_loss(
            spkid_labels, spk_labels, model, attractors, total_n_speakers, permutations
        )
    else:
        spkid_loss = 0
...
def speaker_identification_loss(
    ...
    for i in range(spk_labels.shape[0]):
        ...
        masked_spk_labels = spk_labels[i].to(int) * torch.tensor(
            spkid_labels[i] + [0] * (spk_labels[i].shape[0] - len(spkid_labels[i]))
    ...
    if len(indices) > 0:
        return (
            loss_function(
                model.module.get_speaker_logits(selected_att, indices), indices
            )
            / indices.shape[0]
        )
    else:
...
def get_attractor_quantity_loss(attractors_logits: torch.Tensor, n_speakers: List[int]):
