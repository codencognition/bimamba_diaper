...
def save_checkpoint(
    args, epoch: float, model: torch.nn.Module, optimizer: NoamOpt, loss: torch.Tensor
) -> None:
...
def average_checkpoints(
    device: torch.device, model: torch.nn.Module, models_path: str, epochs: str
) -> torch.nn.Module:
    ...
    for e in epochs:
        ...
        checkpoint = torch.load(
            join(models_path, f"checkpoint_{e}.tar"), map_location=device
        )
...
class VanillaSpeakerLayer(torch.nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, spkid_labels: torch.Tensor = None
    ) -> torch.Tensor:
        return torch.matmul(x, self.weights)


class ArcfaceSpeakerLayer(torch.nn.Module):
    def __init__(self, in_dim: int, out_dim: int, device: torch.device, s=30.0, m=0.30):
        self.device = device
    ...
    def forward(self, x: torch.Tensor, spkid_labels: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.nn.functional.one_hot(spkid_labels, num_classes=self.out_dim)
        inv_mask = 1 - mask
...
class MultiHeadSelfAttention(torch.nn.Module):
    ...
    def __init__(
        self, device: torch.device, n_units: int, h: int, dropout: float
    ) -> None:
    ...
    def __call__(self, x: torch.Tensor, batch_size: int) -> torch.Tensor:
        ...
        scores = torch.matmul(q.permute(0, 2, 1, 3), k.permute(0, 2, 3, 1)) / np.sqrt(
            self.d_k
        )
        # scores: (B, h, T, T)
...
class PositionwiseFeedForward(torch.nn.Module):
    ...
    def __init__(
        self, device: torch.device, n_units: int, d_units: int, dropout: float
    ) -> None:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        return self.linear2(
            F.dropout(F.relu(self.linear1(x)), self.dropout, training=self.training)
        )
...
class PositionalEncoding(torch.nn.Module):
    ...
    def __init__(self, d_model: int, max_len: int, dropout: float = 0.1):
        ...
        div_term = torch.exp(
            torch.arange(0, d_model, 2) * (-math.log(float(2 * max_len)) / d_model)
        )
...
class LinearCombination(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        entropy_term = (
            (weights_normalized * torch.log(weights_normalized)).mean(dim=0).sum()
        )
        return (
            torch.matmul(x.permute(0, 2, 1), weights_normalized).permute(0, 2, 1),
            entropy_term,
        )
...
class AttractorPerceiver(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_pre_crossattention:
            self.pre_crossattention = torch.nn.MultiheadAttention(
                args.d_latents, args.pre_xa_heads, batch_first=True, device=self.device
            )
        else:
        ...
        if self.use_frame_selfattention:
            ...
            self.lnorm_in = torch.nn.LayerNorm(args.d_latents, device=self.device)
            ...
            self.lnorm_out = torch.nn.LayerNorm(args.d_latents, device=self.device)
        ...
        if self.use_posenc:
            self.pos_encoder = PositionalEncoding(args.d_latents, args.posenc_maxlen)
        else:
        ...
        self.latent_attractors = torch.nn.Parameter(
            torch.normal(
        ...
        if self.lat2att == "weighted_average":
            ...
            assert (
                args.n_latents == args.n_attractors
        ...
        if args.speakerid_loss == "arcface":
            self.speaker_layer = ArcfaceSpeakerLayer(
                args.d_latents, args.speakerid_num_speakers, device=self.device
            )
        ...
        if self.context_activations:
            ...
            self.contextualizer_linear = torch.nn.Linear(
                args.d_latents, args.n_attractors, device=self.device
            )
        else:
    ...
    def forward(
        self, inputs: torch.Tensor, args: SimpleNamespace
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        all_frame_embs = []
        ...
        for i in range(self.n_layers):
            ...
            per_frameenclayer_atts_logits.append(per_prcvblock_attractors_logits[-1])
            ...
            if self.condition_frame_encoder:
                if args.length_normalize:
                    ...
                    logits = 20 * torch.matmul(
                        torch.nn.functional.normalize(e.clone(), dim=-1),
                        atts.permute(0, 2, 1),
                    )
                else:
                ...
                condition = torch.matmul(
                    torch.matmul(torch.sigmoid(logits), atts), self.W
                )
                e += condition
        ...
        for i in range(len(per_prcvblock_latents)):
            if args.attractor_frame_comparison == "xattention":
                ys_logits = self.frame_activate(
                    per_prcvblock_attractors[i], all_frame_embs[-1], all_frame_embs[-1]
                )[1].transpose(2, 1)
            else:
                if args.length_normalize:
                    ys_logits = 20 * torch.matmul(
                        ...
                        torch.nn.functional.normalize(
                            per_prcvblock_attractors[i].permute(0, 2, 1), dim=1
                        ),
                    )
                else:
        ...
        all_frame_embs = torch.permute(torch.stack(all_frame_embs), (1, 2, 3, 0))
        per_frameenclayer_ys_logits = torch.permute(
            torch.stack(per_frameenclayer_ys_logits), (1, 2, 3, 0)
        )
        per_frameenclayer_atts_logits = torch.permute(
            torch.stack(per_frameenclayer_atts_logits), (1, 2, 0)
        )
        per_frameenclayer_atts = torch.permute(
            torch.stack(per_frameenclayer_atts), (1, 2, 3, 0)
        )
        per_prcvblock_ys_logits = torch.permute(
            torch.stack(per_prcvblock_ys_logits), (1, 2, 3, 0)
        )
        per_prcvblock_attractors_logits = torch.permute(
            torch.stack(per_prcvblock_attractors_logits), (1, 2, 0)
        )
        per_prcvblock_attractors = torch.permute(
            torch.stack(per_prcvblock_attractors), (1, 2, 3, 0)
        )
        per_prcvblock_l2a_entropy_term = torch.stack(per_prcvblock_l2a_entropy_term)
        per_prcvblock_latents = torch.permute(per_prcvblock_latents, (1, 2, 3, 0))
    ...
    def get_speaker_logits(
        self, attractors: torch.Tensor, spkid_labels: torch.Tensor
    ) -> torch.Tensor:
    ...
    def get_attractors(self, frame_embs: torch.Tensor):
        ...
        if self.use_pre_crossattention:
            latents = self.pre_crossattention(latents, frame_embs, frame_embs)[0]
        per_prcvblock_latents = self.encoder_attractors(latents, inputs=frame_embs)
        if self.device != torch.device("cpu"):
            self.encoder_attractors = self.encoder_attractors.to(torch.device("cuda"))
            per_prcvblock_latents = per_prcvblock_latents.to(torch.device("cuda"))
        for i in range(len(per_prcvblock_latents)):
            ...
            per_prcvblock_logits.append(torch.squeeze(self.counter(attractors_i), 2))
        return (
            per_prcvblock_latents,
            per_prcvblock_attractors,
            per_prcvblock_logits,
    ...
    def count_parameters(self, args: SimpleNamespace):
        ...
        frame_encoder_modules = (
            ...
            + [
                getattr(self, "{}{:d}".format("lnorm1_", i))
                for i in range(self.n_layers)
            ]
            + [
                getattr(self, "{}{:d}".format("self_att_", i))
                for i in range(self.n_layers)
            ]
            + [
                getattr(self, "{}{:d}".format("lnorm2_", i))
                for i in range(self.n_layers)
            ]
        ...
        modules = [
            self.pre_crossattention,
            self.pos_encoder,
            self.latent_attractors,
            self.encoder_attractors,
            self.latents2attractors,
            self.counter,
            self.frame_activate,
            self.speaker_layer,
            self.contextualizer_lstm,
