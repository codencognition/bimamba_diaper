...
def _convert(batch: List[Tuple[torch.Tensor, torch.Tensor, str]]) -> Dict[str, Any]:
    return {
...
def get_training_dataloaders(args: SimpleNamespace) -> Tuple[DataLoader, DataLoader]:
    if args.gpu >= 1:
    ...
    if args.train_features_dir is None and args.valid_features_dir is None:
        assert not (args.train_data_dir is None) and not (
            args.valid_data_dir is None
        ), "--features-dir or \
        ...
        train_set = KaldiDiarizationDataset(
            ...
            n_speakers=min(
                args.num_speakers, args.n_attractors
            ),  # read up to n_attractors speakers
        ...
        assert not (args.train_features_dir is None) and not (
            args.valid_features_dir is None
        ), "--train-features-dir and --valid-features-dir or \
            --train-data-dir and --valid-data-dir must be defined"
        train_set = PrecomputedDiarizationDataset(
            features_dir=args.train_features_dir, batch_size=args.train_batchsize
        )
        dev_set = PrecomputedDiarizationDataset(
            features_dir=args.valid_features_dir, batch_size=args.dev_batchsize
        )
...
def parse_arguments() -> SimpleNamespace:
    ...
    parser.add_argument(
        "--activation-loss-BCE-weight",
        default=1.0,
        type=float,
        help="weighting parameter for activation loss BCE",
    )
    parser.add_argument(
        "--activation-loss-DER-weight",
        default=0.0,
        type=float,
        help="weighting parameter for activation loss DER",
    )
    parser.add_argument(
        "--attractor-existence-loss-weight",
        default=1.0,
        type=float,
        help="weighting parameter",
    )
    parser.add_argument(
        "--attractor-frame-comparison",
        default="dotprod",
        type=str,
        choices=["dotprod", "xattention"],
    ...
    parser.add_argument(
        "--d-latents", type=int, default=None, help="dimension of latents"
    )
    parser.add_argument(
        "--detach-attractor-loss",
        type=bool,
    ...
    parser.add_argument(
        "--dropout", type=float, help="dropout for the rest of the model"
    )
    parser.add_argument(
        "--dropout_attractors", type=float, help="attention dropout for attractors path"
    )
    parser.add_argument(
        "--dropout_frames",
        type=float,
    ...
    parser.add_argument(
        "--input-transform",
        default="",
        choices=["logmel", "logmel_meannorm", "logmel_meanvarnorm"],
    ...
    parser.add_argument("--max-epochs", type=int, help="Max. number of epochs to train")
    parser.add_argument(
        "--min-length",
        default=0,
        type=int,
        help="Minimum number of frames for the sequences" " after downsampling.",
    )
    parser.add_argument(
        "--model-type",
        default="AttractorsPath",
        help="Type of model (for now only AttractorsPath)",
    )
    parser.add_argument("--n-attractors", type=int, help="Number of attractors to use")
    parser.add_argument(
        "--n-embeddings", type=int, default=1, help="Number of embeddings to use"
    )
    parser.add_argument(
        "--n-blocks-attractors",
        type=int,
    ...
    parser.add_argument(
        "--n-xa-heads-out", type=int, help="number of output cross-attention heads"
    )
    parser.add_argument(
        "--n-speakers-softmax",
        type=int,
    ...
    parser.add_argument(
        "--num-frames", type=int, help="number of frames in one utterance"
    )
    parser.add_argument(
        "--num-speakers", type=int, help="maximum number of speakers allowed"
    )
    parser.add_argument(
        "--num-workers",
        default=1,
    ...
    parser.add_argument(
        "--pre-xa-heads", type=int, help="number of pre-Perceiver cross-attention heads"
    )
    ...
    parser.add_argument(
        "--speakerid-loss", type=str, default="", choices=["arcface", "vanilla"]
    )
    parser.add_argument(
        "--speakerid-loss-weight",
        default=0.0,
        type=float,
    ...
    parser.add_argument(
        "--train-data-dir", default=None, help="kaldi-style data dir used for training."
    )
    parser.add_argument(
        "--train-features-dir",
        default=None,
...
if __name__ == "__main__":
    ...
    if os.path.isfile(os.path.join(args.output_path, "models", "checkpoint_0.tar")):
    ...
    for epoch in range(int(round(init_epoch)), args.max_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss, acum_train_metrics = compute_loss_and_metrics(
                model, labels, features, n_speakers, spkids, acum_train_metrics, args
            )
            if i % args.log_report_batches_num == (args.log_report_batches_num - 1):
                ...
                writer.add_scalar("lrate", get_rate(optimizer), train_batches_qty)
                acum_train_metrics = reset_metrics(acum_train_metrics)
                print(
                    f"Epoch {epoch+1}/{args.max_epochs} | Batch {i+1} | Loss: {loss.item():.4f} | LR: {get_rate(optimizer):.6f}"
                )
            optimizer.zero_grad()
            loss.backward()
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(dev_loader):
                ...
                _, acum_dev_metrics = compute_loss_and_metrics(
                    model, labels, features, n_speakers, spkids, acum_dev_metrics, args
                )
        for k in acum_dev_metrics.keys():
            writer.add_scalar(
                f"dev_{k}",
                acum_dev_metrics[k] / dev_batches_qty,
                epoch * dev_batches_qty + i,
            )
        n = dev_batches_qty
        print(
            f"=== Dev - Epoch {epoch+1} | Loss: {acum_dev_metrics['loss']/n:.4f} | DER: {acum_dev_metrics['DER']/n:.2f}% (miss: {acum_dev_metrics['DER_miss']/n:.2f}%, FA: {acum_dev_metrics['DER_FA']/n:.2f}%, conf: {acum_dev_metrics['DER_conf']/n:.2f}%) ==="
        )
